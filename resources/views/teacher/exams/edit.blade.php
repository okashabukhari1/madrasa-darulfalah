...
<div class="mb-4 d-flex justify-content-between align-items-center" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
    <div>
        <h2 style="margin: 0; font-family: var(--font-heading); color: var(--gray-800);">Edit Evaluation</h2>
        <p style="margin: 0; color: var(--gray-500); font-size: 13px;">Editing record for {{ $exam->student->user->name }}</p>
    </div>
...
<div class="db-card" style="background: #fff; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); border: 1px solid var(--gray-200); overflow: hidden; padding: 30px;">
    <form method="POST" action="{{ route('teacher.exams.update', $exam) }}">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 24px;">
            ...
            <div class="form-group" style="margin-bottom:0;">
                <label>Date *</label>
                <input type="date" name="date" class="form-control" value="{{ old('date', $exam->date->format('Y-m-d')) }}" max="{{ date('Y-m-d') }}" required>
