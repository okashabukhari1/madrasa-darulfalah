...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (lightbox && galleryItems.length) {
    ...
    function openLightbox(idx) {
      ...
      if (lightboxImg) {
        lightboxImg.querySelector('.lb-emoji').textContent = item.dataset.emoji || '🕌';
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function(e) {
      ...
      if (valid) {
        const sMsg = document.getElementById('contact-success');
        if (sMsg) sMsg.style.display = 'block';
        contactForm.reset();
        setTimeout(() => { if (sMsg) sMsg.style.display = 'none'; }, 6000);
      }
    });
  }
